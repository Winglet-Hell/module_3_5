def get_multiplied_digits(number):
    ...
    if len(str_number) > 1:
        ...
        print(f"Произведение для числа {number} = {first} * результат рекурсии ({int(str_number[1:])}) = {result}")
...
result_funk = get_multiplied_digits(40203)
print(f"\nИтоговый результат: {result_funk}")
